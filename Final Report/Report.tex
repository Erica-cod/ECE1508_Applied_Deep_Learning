...
In this section, we present numerical experiments for both encoder-based BERT variants and the decoder-based Qwen2.5--7B model on the Recipe-MPR test set.

\subsection{BERT Variant Results}

Table~\ref{table:bert-accuracy} summarizes the test accuracy for the fine-tuned BERT variants.
...
\midrule
RoBERTa-base & 125M & 92.40 \\
BERT-large & 340M & 91.40 \\
DistilBERT & 66M & 88.80 \\
\bottomrule
\end{tabular}
\caption{Test accuracy of fine-tuned BERT variants on Recipe-MPR.}
\label{table:bert-accuracy}
\end{table}

Table~\ref{table:bert-query-type} reports accuracy by reasoning category for RoBERTa-base, comparing the baseline (non-fine-tuned) model against the fine-tuned version.

\begin{table}[h]
\centering
\begin{tabular}{l c c c}
\toprule
\textbf{Query Type} & \textbf{Baseline (\%)} & \textbf{Fine-tuned (\%)} & \textbf{Improvement} \\
\midrule
Analogical   & 20.00 & 100.00 & +80.00 \\
Negated      & 23.85 & 99.08 & +75.23 \\
Temporal     & 18.75 & 96.88 & +78.13 \\
Specific     & 26.49 & 94.70 & +68.21 \\
Commonsense  & 25.37 & 91.42 & +66.05 \\
\midrule
Overall      & 23.80 & 92.40 & +68.60 \\
\bottomrule
\end{tabular}
\caption{Accuracy by reasoning category for RoBERTa-base: baseline vs.\ fine-tuned.}
\label{table:bert-query-type}
\end{table}

All three models exceed the 65\% accuracy target by a substantial margin. RoBERTa-base achieves the highest accuracy (92.4\%) despite having fewer parameters than BERT-large, benefiting from improved pretraining procedures. Training times range from 1.7 minutes for DistilBERT to 7.8 minutes for BERT-large.
...
\subsubsection{Accuracy by Query Type}
...
\subsubsection{Training Cost and Efficiency}
...
\subsubsection{Qualitative Error Corrections}
