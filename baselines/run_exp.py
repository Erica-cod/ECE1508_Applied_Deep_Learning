...
LLAMA31_MODEL_ID = os.environ.get("LLAMA31_MODEL_ID", "meta-llama/Meta-Llama-3.1-8B-Instruct")

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
DATA_PATH = os.path.join(SCRIPT_DIR, "..", "data", "500QA.json")

LM_ALIASES = {'gpt2':'gpt2',
...
'llama-3.1':LLAMA31_MODEL_ID,
'gpt3-ada':'embeddings_ada_with_aspects.json'}

OPT_FINETUNED_PATH = os.path.expanduser("~/models/opt-finetuned")
if os.path.isdir(OPT_FINETUNED_PATH):
    LM_ALIASES['opt-finetuned'] = OPT_FINETUNED_PATH

BERT_FINETUNED_PATH = os.path.expanduser("~/models/hub/bert-finetuned")
if os.path.isdir(BERT_FINETUNED_PATH):
    LM_ALIASES['bert-finetuned'] = BERT_FINETUNED_PATH
...
def perform_experiment(fold,args):
    ...
    folds_path = os.path.abspath(args.folds_path)
    ...
    train_splits, val_splits, test_splits = load_data(exp_config['folds_path'],data_path=DATA_PATH)
    ...
    if exp_config['monolithic']:
        if exp_config['LM'] == 'DV3':
            ...
            resolved_lm = LM_ALIASES.get(exp_config['LM'], exp_config['LM'])
            if exp_config['type'] == 'dense':
                predictions = dense_pred(test_data,resolved_lm)
            elif exp_config['type'] == 'ZS':
                predictions = ZS_pred(test_data,resolved_lm)
            elif exp_config['type'] == 'FS':
                predictions = FS_pred(train_data,test_data,resolved_lm, prompt_size=exp_config['FS_num'])
        ...
        if exp_config['LM'] == 'DV3':
            ...
            resolved_lm = LM_ALIASES.get(exp_config['LM'], exp_config['LM'])
            if exp_config['type'] == 'dense':
                predictions = aspect_dense_pred(test_data,resolved_lm,agg_fcn=agg)
            elif exp_config['type'] == 'ZS':
                predictions = aspect_ZS_pred(test_data,resolved_lm,agg_fcn=agg)
            elif exp_config['type'] == 'FS':
                predictions = aspect_FS_pred(train_data,test_data,resolved_lm,agg_fcn=agg, prompt_size=exp_config['FS_num'])
