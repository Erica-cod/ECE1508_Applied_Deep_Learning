#!/usr/bin/env python
"""
Train a local OPT model on the Recipe-MPR dataset.

This script expects:
  * A cached OPT checkpoint under --model-path (defaults to ~/models/hub/models--facebook--opt-1.3b)
  * The Recipe-MPR JSON data file at --dataset-path (defaults to data/500QA.json)

Usage (example):
    python scripts/finetune_opt_recipe_mpr.py \
        --output-dir runs/opt-recipe-mpr \
        --num-train-epochs 3 \
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fine-tune OPT on the Recipe-MPR dataset.")
    parser.add_argument(
        "--model-path",
        type=str,
        default="~/models/hub/models--facebook--opt-1.3b",
        help="Path to the local OPT checkpoint directory (a snapshot or root cache folder).",
    )
    parser.add_argument(
        "--tokenizer-path",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Optional evaluation frequency in steps when using 'steps' eval strategy.",
    )
    parser.add_argument(
        "--save-strategy",
        type=str,
        ...
        help="Checkpointing strategy (if unsupported it will be ignored).",
    )
    parser.add_argument(
        "--save-steps",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Limit the total number of checkpoints.",
    )
    parser.add_argument(
        "--eval-strategy",
        type=str,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Keep track of eval metrics and load the best checkpoint when supported.",
    )
    parser.add_argument(
        "--no-eval",
        action="store_true",
...
def validate_precision_flags(args: argparse.Namespace) -> None:
    ...
    if args.fp16 and not cuda_available:
        raise EnvironmentError(
            "FP16 training requires a CUDA-capable GPU. "
            "Rerun without --fp16 or enable CUDA."
        )
    if args.bf16 and not (cuda_available or mps_available):
        raise EnvironmentError(
            "BF16 training requires CUDA or Apple MPS hardware. "
            "Rerun without --bf16 or enable supported hardware."
        )
...
def resolve_model_and_tokenizer_paths(
    ...
    """
    Expand the provided paths and locate the snapshot folders that contain model
    weights, config, and tokenizer assets. When tokenizer_path is omitted, it falls
    back to the directory that supplies the model weights.
    """
    ...
    if model_snapshot is None:
        raise FileNotFoundError(
            f"Could not locate model weights under {expanded_model}. "
            "Ensure the directory contains pytorch_model.bin or model.safetensors."
        )
    ...
    if tokenizer_snapshot is None:
        raise FileNotFoundError(
            "Unable to locate tokenizer assets. "
            "Provide --tokenizer-path explicitly or ensure the cache folder includes tokenizer files."
        )
    if config_snapshot is None:
        raise FileNotFoundError(
            "Unable to locate config.json for the model. "
            "Ensure the cache directory includes configuration files."
        )
...
def load_recipe_mpr(dataset_path: str) -> List[Dict[str, str]]:
    ...
    for entry in raw_examples:
        ...
        option_items = list(options.items())  # preserves ordering from JSON
...
class CompletionDataCollator:
    ...
    def __call__(self, features: List[Dict[str, List[int]]]) -> Dict[str, torch.Tensor]:
        ...
        if self.pad_to_multiple_of is not None:
            if max_seq_len % self.pad_to_multiple_of != 0:
                max_seq_len = (
...
def main():
    ...
    eval_len = len(dataset_dict["validation"]) if "validation" in dataset_dict else 0
    logging.info("Prepared dataset splits: %d train / %d eval examples", train_len, eval_len)
    ...
    elif args.bf16 and (torch.cuda.is_available() or getattr(torch.backends, "mps", None) and torch.backends.mps.is_available()):
    ...
    metrics["train_perplexity"] = float(torch.exp(torch.tensor(metrics["train_loss"]))) if "train_loss" in metrics else None
